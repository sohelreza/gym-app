...
import { sAdminDietTypeListSetScrollPosition } from "../../Redux";
...
import "./SAdminDietTypeList.css";

class SAdminDietTypeList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      name: "",
  ...
  componentDidMount() {
    ...
    sAdminDietTypeApi.sAdminGetDietTypeList(formattedToken).then((response) => {
  ...
  handleSubmit(e) {
    ...
    sAdminDietTypeApi
      .sAdminAddDietType(newDietTypeData, formattedToken)
      .then((response) => {
        this.setState({
  ...
  deleteData(dietTypeId) {
    const { formattedToken, sAdminDietTypeListSetScrollPosition } = this.props;

    sAdminDietTypeApi
      .sAdminDeleteDietType(dietTypeId, formattedToken)
      .then((response) => {
        this.setState({ dietTypeList: response.data });
        sAdminDietTypeListSetScrollPosition(
          window.pageXOffset,
          window.pageYOffset
        );
      });
  ...
  getDietTypeList() {
    ...
    const { sAdminDietTypeListSetScrollPosition } = this.props;
                ...
                <tr key={index}>
                  ...
                  <td className="align-middle">
                    <Link
                      to={{
                        pathname:
                          routesList.S_ADMIN_GET_DIET_TYPE_DETAILS + "/" + _id,
                    ...
                    <Link
                      to={{
                        pathname:
                          routesList.S_ADMIN_EDIT_DIET_TYPE_DETAILS + "/" + _id,
                        state: { data: "test data" },
                      }}
...
const mapStateToProps = (state) => {
  ...
  const positionX = state.sAdminDietTypeReducer.dietTypeListPositionX;
  const positionY = state.sAdminDietTypeReducer.dietTypeListPositionY;
...
const mapDispatchToProps = (dispatch) => {
  return {
    sAdminDietTypeListSetScrollPosition: (positionX, positionY) =>
      dispatch(sAdminDietTypeListSetScrollPosition(positionX, positionY)),
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(SAdminDietTypeList);
